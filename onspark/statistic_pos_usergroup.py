	...
	try:
		...
		if bss.has_key(key):
			...
			if 120.02<=lng<120.48 and 30.15<=lat<=30.42:
				gx, gy = int((lng-120.02)/(120.48-120.02)*90), int((lat-30.15)/(30.42-30.15)*60)
...
if __name__ == "__main__":
	...
	counts = lines.map(lambda x : extract(x)) \
			...
			.groupByKey() \
			.sortByKey() \
			.map(lambda x : str(x[0][0])+" "+str(x[0][1])+" "+" ".join(list(x[1])))
	output = counts.saveAsTextFile("./hangzhou/CFF/{0}_pos_30min_usergroup_grid.csv".format(filename))
