...
# user_center
def center(x):
	cx, cy = float(sum([int(i.split(",")[0]) for i in x]))/len(x), float(sum([int(i.split(",")[1]) for i in x]))/len(x) 
	return str(round(cx,2))+","+str(round(cy,2))
...
if __name__ == "__main__":
	...
	counts = lines.map(lambda x : extract(x)) \
			...
			.distinct() \
			.groupByKey() \
			.map(lambda x : (x[0],center(list(x[1])))) \
			.sortByKey() \
			.map(lambda x : str(x[0][0])+" "+str(x[0][1])+" "+str(x[1]))
	output = counts.saveAsTextFile("./hangzhou/CFF/{0}_user_30min_center.csv".format(filename))
